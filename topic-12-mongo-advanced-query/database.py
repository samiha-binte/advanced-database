...
def create_pet(data):
    ...
    kind = kind_collection.find_one({"_id": kind_id})
    if kind is None:
        raise ValueError("Cannot create pet: the specified kind does not exist.")
...
def delete_kind(id):
    ...
    pets_collection = pets_db.pets_collection
    kind_id = ObjectId(id)
    if pets_collection.find_one({"kind_id": kind_id}):
        return "Cannot delete kind: there are pets that depend on it."
    kind_collection.delete_one({"_id": kind_id})
